...
if (-not (Get-Command wsl.exe -ErrorAction SilentlyContinue)) {
    throw "wsl.exe was not found. Install WSL and a Linux distribution before regenerating the ISO."
}
...
& wsl.exe @distroArgs --exec bash -lc "export PATH='$cleanPath'; cd '$linuxRepoQuoted' && bash scripts/make-iso.sh"
